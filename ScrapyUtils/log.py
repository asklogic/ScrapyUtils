...
def initial_logger():
    ...
    """命令日志列表"""
    command_format = Formatter('[%(name)s]: %(message)s', r'%H:%M:%S')
    """命令日志格式"""

    # 设置命令日志格式, 通用日志
    [set_stream_logger(logger_name, command_format) for logger_name in command_loggers + common_loggers]

    core_loggers = ['preload', 'load', 'persist', 'engine']
    """爬虫核心的日志列表"""
    core_format = Formatter('%(asctime)s (%(levelname)s) [%(name)s]: %(message)s', '%H:%M:%S')
    """爬虫核心的日志格式"""
    ...
    """组件以及其他通用组件的日志"""
    component_format = Formatter('%(asctime)s (%(levelname)s) [%(name)s] in %(funcName)s: %(message)s', )
    """组件的日志格式"""
